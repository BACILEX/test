...
Чрезвычайно часто возникает необходимость создать представление, параметризованное некоторым значением или несколькими. К примеру, представление, выводящее данные о клиенте, может иметь один параметр -- id пользователя, после чего оно может обратиться к базе данных с запросом данных о клиенте с указанным id, получить информацию о нем и передать полученные данные в шаблон для отображения. Поскольку представления являются обработчиками HTTP-запросов, необходимо, чтобы запрос содержал значения этих параметров, которые будут переданы представлению.
...
После этого следует произвести инициализацию механизмов библиотеки, путем включения в файл ``__init__.py`` кода создания объекта класса ``LoginManager``:
...
Полезно было бы предусмотреть механизм, который при попытке доступа к ресурсу, требующему аутентификации, вместо возвращения ошибки перенаправлял бы на страницу входа в систему, чтобы пользователь мог ввести логин и пароль, после чего был бы перенаправлял бы обратно к ресурсу, к которому был запрошен доступ. Для этого необходимо внести два изменения.

* Библиотеке Flask-Login нужно указать на то, какое из представлений является представлением входа в систему, именно на это представление будут перенаправляться неаутентифицированные пользователи при попытке доступа к ресурсам с декоратором ``login_required``. Это достигается заданием значения атрибута ``login_view`` у объекта класса ``LoginManager``, созданного в файле ``__init__.py``. В данном примере полагается, что представление называется ``login``, как оно названо в предыдущем подразделе пособия.
...
* В код представления входа в систему нужно внести обработку переданного URL-параметра ``next``. Дело в том, что при перенаправлении на страницу входа в систему библиотека Flask-Login формирует ссылку, позволяющую после осуществления входа автоматически вернуться обратно, запоминая адрес запрошенного ресурса в параметре URL с названием ``next``. При этом ссылка может иметь вид ``/login?next=%2Fprofile``. Это означает, что после удачного входа в систему пользователь будет перенаправлен к ресурсу ``/profile`` (с помощью кода ``%2F`` в URL помещается прямая косая черта ``/``).
...
.. code-block:: python
    ...
    def login():
        login_form = LoginForm()
        if login_form.validate_on_submit():
            # проверка имени пользователи и пароля
            # перенаправление на повторный вход в случае неудачи
            # вход в систему в случае успеха
...
В данном примере мы получаем значение параметра ``next`` из URL с помощью обращения к словарю ``args`` объекта ``request``, как это демонстрировалось ранее в данном пособии, после чего проверяем это значение. Если оно отсутствует (пользователь просто перешел по ссылке на страницу входа, и не был на нее перенаправлен), то к качестве адреса для перенаправления после входа выступает представление ``index``. То же самое происходит при выполнении условия ``urlsplit(next).netloc != ''``. Сама эта проверка является необязательной, но крайне желательной для веб-приложений с точки зрения безопасности, поскольку защищает от уязвимости открытого перенаправления (open redirect) [#openredirect]_. При наличии этой уязвимости пользователь может воспользоваться зловредной ссылкой вида ``/login?next=https%3A%2F%2Fguap.ru``, и после удачного ввода имени пользователя и пароля будет перенаправлен на сайт злоумышленника (``https://guap.ru``), не подозревая об этом. Функция ``urlsplit`` из стандартной библиотеки Python разбивает URL на составляющие, и поле ``netloc`` после этого разбиения содержит хост URL (доменное имя или IP-адрес). В случае, если это поле не пустое, есть вероятность, что ссылка может вести на другой сайт, этого допустить нельзя, поэтому такое перенаправление также заменяется перенаправлением на представление ``index``.
...
.. [#openredirect] https://habr.com/ru/companies/otus/articles/511428/
...
Стоит отметить, что Tembo предоставляет веб-интерфейс для написания команд на SQL, а также средство просмотра таблиц и редактирования их содержимого (страница SQL Editor). К сожалению, на момент написания пособия онлайн редактирование и просмотр таблиц не очень полезны, так как способны работать только с базой данных ``postgres``.
...
- Приложение развертывается в эфемерной файловой системе, отсутствует доступ к персистентному хранилищу. Иными словами приложение не может сохранять какую-либо информацию на жесткий диск (кроме как в базу данных), и все изменения, которые работающее приложение сделает в файловой системе, будут потеряны при следующем развертывании.
...
- Для развернутого сервиса не предоставляется доступ по SSH.
...
При выполнении этих команд должно быть активировано виртуальное окружение, используемое для запуска проекта на компьютере разработчика. 
